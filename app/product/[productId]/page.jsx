...
export default function ProductDetailPage() {
  ...
  const [formFilled, setFormFilled] = useState(false); // State to track if showModalPDF form is filled
  ...
  const handleData = (e) => {
    ...
    if (formData2.name && formData2.contact) {
      setFormFilled(true);
    } else {
      setFormFilled(false);
    }
        ...
        <div className="fixed z-20 inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div id="form-response" className="bg-white rounded shadow-lg max-w-md">
            ...
            <form>
              ...
              <div className="flex items-center justify-between">
                <button
                  onClick={formFilled ? handleDownloadPdf : undefined} // Only execute handleDownloadPdf if formFilled is true
                  className={`w-full bg-blue-500 mt-4 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ${formFilled ? '' : 'opacity-50 cursor-not-allowed'}`}
                  type="button"
                >
                  Download Now
                </button>
